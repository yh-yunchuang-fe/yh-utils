...
function setAndroid() {
    var bridge = {
        default: this, callHandler: function (b, a, c) {
            var e = "";
            ...
            return this.callHandler("_dsb.hasNativeMethod", {name: b, type: a || "all"})
    ...
    !function () {
        if (!window._dsf) {
            var b = {
                _dsf: {_obs: {}}, _dsaf: {_obs: {}}, dscb: 0, WebViewJavascriptBridge: bridge, close: function () {
...
const jsBridge = {
    ...
    addBridgeProperty: function(bridge) {
        const dependencies = ['getUserInfo', 'login', 'refreshToken', 'getLocation', 'share', 'setShareConfig',
                                'getAppInfo', 'track', 'closeWebPage', 'request', 'getEnv', 'getNetwork',
                                'setNativeStore', 'getNativeStore', 'removeNativeStore', 'pay',
        ...
        // if (this.openWebPage) {
            this.openWebPage = (url) => {
                window.location.href = `myyh://yhlife.com/show/web?url=${encodeURIComponent(url)}`
            }
        // }
